...
WITH GEO_NAMES AS(
  ...
  FROM(
      SELECT *
   , ROW_NUMBER() OVER(PARTITION BY "validity_date","validity_time_utc" ORDER BY validity_time_utc DESC) AS RN
      FROM STAGE.stg_monthly_forecasts
  ) A WHERE RN=1
), monthly_air_data_forecasts AS(
    SELECT * FROM MONTHLY_FORECASTS_FILETERED
), EXPORT AS(
  SELECT
